...
<body class="bg-gray-50 min-h-screen flex items-center justify-center">
    ...
    <div class="w-full max-w-4xl bg-white rounded-2xl shadow-lg overflow-hidden grid md:grid-cols-2">
        ...
        <div
            class="hidden md:flex flex-col justify-center items-center bg-gradient-to-b from-emerald-700 to-emerald-500 text-white p-10">
            <a href="{{ route('accueil') }}">
                <h1 class="text-4xl font-bold">FootQuartier</h1>
            </a>
        ...
        <div class="p-6 flex flex-col justify-center">
            ...
            <p class="text-gray-600 mb-4">Créez votre compte <span><a href="{{ route('accueil') }}"
                        class="text-emerald-600 font-semibold hover:underline">FootQuartier</a></span> et commencez à
                réserver vos terrains facilement.</p>
            ...
            <form action="{{ route('inscription') }}" method="POST" class="space-y-4">
                ...
                @if (\App\Models\User::count() > 1)
                    <select name="role"
                        ...
                        <option value="joueur" {{ old('role', $role ?? '') == 'joueur' ? 'selected' : '' }}>
                            Joueur
                        </option>

                        <option value="moderateur" {{ old('role', $role ?? '') == 'moderateur' ? 'selected' : '' }}>
                            Modérateur
                        </option>

                    </select>
                    <div id="stripe-field" class="hidden">
                        <input type="text" name="stripe_id" placeholder="Stripe Account ID (ex: acct_xxxxx)"
                            value="{{ old('stripe_id') }}"
                            class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-400 focus:outline-none">
                        @error('stripe_id')
                            <p class="text-red-500 text-sm">{{ $message }}</p>
    ...
    <script>
        ...
        const stripeField = document.getElementById('stripe-field');

        function toggleStripe() {
            stripeField.classList.toggle('hidden', roleSelect.value !== 'moderateur');
        }

        roleSelect.addEventListener('change', toggleStripe);
        toggleStripe(); 
    </script>
