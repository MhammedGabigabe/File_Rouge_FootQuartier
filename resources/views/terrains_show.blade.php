...
<head>
    ...
    <style>
        ...
        #map { height: 100%; width: 100%; min-height: 320px; border-radius: 0 1rem 1rem 0; z-index: 0; }
        .badge-equip {
        ...
        .sidebar-nav-link.active { background: #d1fae5; color: #065f46; font-weight: 600; }
    </style>
</head>
...
<body class="bg-gray-50 text-gray-800 min-h-screen overflow-hidden @auth flex  @endauth">
        ...
        <aside class="w-64 bg-white shadow-lg flex flex-col justify-between fixed h-full z-40">
            <div>
                ...
                <nav class="p-4 space-y-1">
                    <a href="{{ route('joueur.dashboard') }}" class="sidebar-nav-link {{ request()->routeIs('joueur.dashboard') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('joueur.reservations') }}" class="sidebar-nav-link {{ request()->routeIs('joueur.reservations') ? 'active' : '' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        Mes réservations
                    </a>
                    <a href="{{ route('annonces') }}" class="sidebar-nav-link {{ request()->routeIs('annonces') ? 'active' : '' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                        </svg>
                        Trouver un match
                    </a>
                    <a href="{{ route('joueur.participations') }}" class="sidebar-nav-link {{ request()->routeIs('joueur.participations') ? 'active' : '' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
                        </svg>
                        Mes participations
                    </a>
                    <a href="{{ route('joueur.points') }}" class="sidebar-nav-link {{ request()->routeIs('joueur.points') ? 'active' : '' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        Mes points
                    </a>
                    <a href="{{ route('joueur.historique') }}" class="sidebar-nav-link {{ request()->routeIs('joueur.historique') ? 'active' : '' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                        </svg>
                        Historique
                    </a>
                    <a href="{{ route('joueur.notifications') }}" class="sidebar-nav-link {{ request()->routeIs('joueur.notifications') ? 'active' : '' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <nav class="fixed top-0 w-full bg-white shadow-sm z-50 mb-6">
            <div class="max-w-6xl mx-auto flex justify-between items-center px-6 py-4">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('login') }}" class="px-4 py-2 text-sm text-gray-700 hover:text-emerald-600 font-medium">Se connecter</a>
                    <a href="{{ route('inscription') }}" class="px-4 py-2 bg-emerald-600 text-white text-sm rounded-lg hover:bg-emerald-700 font-medium">S'inscrire</a>
                </div>
            </div>
        </nav>
    @endguest
    ...
    <div class="flex-1 min-w-0 @auth ml-64 mt-10 p-2 @endauth @guest pt-24 @endguest">
        <main class="p-2 pb-16 max-w-5xl mx-auto ">
            ...
            <a href="{{ route('terrains') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-emerald-600 mb-4 transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex flex-col lg:flex-row gap-6">
                    ...
                    <div class="flex-1 flex flex-col">
                        ...
                        <div class="relative">
                            ...
                            @endif
                            <span class="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-emerald-700 text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">
                                {{ $terrain->capacite }}v{{ $terrain->capacite }}
                            </span>
                            <span class="absolute top-3 right-3 bg-emerald-600 text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">
                                {{ $terrain->prix }} DH/h
                            </span>
                        </div>
                        ...
                        <div class="p-5 flex flex-col flex-1">
                            ...
                            @php
                                ...
                                $equipsLabels = [
                                    'vestiaires' => 'Vestiaires', 'eclairage' => 'Éclairage',
                                    'gazon_synthetique' => 'Gazon synthétique', 'parking' => 'Parking', 'buvette' => 'Buvette',
                                ];
                            @endphp
                            @if (count($equips) > 0)
