...
<head>
    ...
    <style>
        ...
        .badge-ouverte { background: #d1fae5; color: #065f46; }
        .badge-fermee { background: #fee2e2; color: #991b1b; }
        .badge-complete { background: #fef9c3; color: #854d0e; }
        ...
        .annonce-card { animation: fadeUp 0.35s ease both; }
        .annonce-card:nth-child(1) { animation-delay: 0.05s; }
        .annonce-card:nth-child(2) { animation-delay: 0.10s; }
        .annonce-card:nth-child(3) { animation-delay: 0.15s; }
        .annonce-card:nth-child(4) { animation-delay: 0.20s; }
        .annonce-card:nth-child(5) { animation-delay: 0.25s; }
        .annonce-card:nth-child(6) { animation-delay: 0.30s; }
        ...
        .join-btn:hover { opacity: 0.9; transform: scale(1.02); }
        .join-btn:disabled {
            background: #e5e7eb;
            color: #9ca3af;
            cursor: not-allowed;
    ...
    <aside class="w-64 bg-white shadow-lg flex flex-col justify-between fixed h-full z-40">
        <div>
            ...
            <nav class="p-4 space-y-1">
                ...
                <a href="{{ route('joueur.points') }}" class="nav-link {{ request()->routeIs('joueur.points') ? 'active' : '' }}">
                ...
                <a href="{{ route('joueur.notifications') }}" class="nav-link {{ request()->routeIs('joueur.notifications') ? 'active' : '' }}">
                    ...
                    @if(Auth::user()->unreadNotifications->count() > 0)
                        <span class="ml-auto bg-red-500 text-white text-xs rounded-full px-1.5 py-0.5">{{ Auth::user()->unreadNotifications->count() }}</span>
                    @endif
                </a>
    ...
    <div class="flex-1 ml-64">
        <main class="p-6 max-w-6xl mx-auto">
            ...
            <div class="mb-6">
                ...
                <p class="text-gray-500 text-sm mt-1">Rejoignez une annonce ouverte et complétez une équipe près de chez vous.</p>
            </div>
                ...
                <div class="annonce-card bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-4">
                    <div class="flex flex-col md:flex-row md:items-center gap-4">
                            ...
                            <div class="mt-3">
                                <div class="flex justify-between text-xs text-gray-500 mb-1">
                                    ...
                                    <span class="{{ $complet ? 'text-red-500' : 'text-emerald-600' }} font-medium">
                                        {{ $annonce->places_dispo }} place{{ $annonce->places_dispo > 1 ? 's' : '' }} restante{{ $annonce->places_dispo > 1 ? 's' : '' }}
                                    </span>
                                </div>
                                <div class="places-bar">
                                    <div class="places-fill" style="width: {{ $pct }}%; {{ $pct >= 80 ? 'background: linear-gradient(90deg,#f59e0b,#ef4444);' : '' }}"></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col items-end gap-2 flex-shrink-0">
                            @if($estOrganisateur)
                                <span class="text-xs text-emerald-700 font-semibold bg-emerald-50 px-3 py-1.5 rounded-full">Votre annonce</span>
                            @elseif($dejaParticipe)
                                <span class="text-xs text-blue-700 font-semibold bg-blue-50 px-3 py-1.5 rounded-full flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                    Inscrit
                                </span>
                                <form method="POST" action="{{ route('participations.destroy', $annonce->id) }}">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="text-xs text-red-500 hover:text-red-700 underline">Se désinscrire</button>
                                </form>
        ...
        <div id="modal-box" onclick="event.stopPropagation()">
            ...
            <div class="bg-emerald-50 rounded-xl p-4 mb-5 space-y-2">
                <div class="flex gap-2 text-sm">
                    <svg class="w-4 h-4 text-emerald-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7"/></svg>
                    <div>
                ...
                <div class="flex gap-2 text-sm">
                    <svg class="w-4 h-4 text-emerald-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10"/></svg>
                    <div>
                ...
                <div class="flex gap-2 text-sm">
                    <svg class="w-4 h-4 text-emerald-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857"/></svg>
                    <div>
            ...
            <p class="text-sm text-gray-500 mb-5">En rejoignant ce match, vous vous engagez à être présent. Votre place sera confirmée immédiatement.</p>

            <form id="modal-form" method="POST" action="">
                @csrf
                <div class="flex gap-3">
                    <button type="button" onclick="closeModal()" class="flex-1 border border-gray-200 rounded-xl py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition font-medium">
                        Annuler
                    </button>
