    ...
    <div class="flex-1 ml-64 overflow-y-auto">
        <main class="p-6 pb-16">
            ...
            <div class="mb-6">
                ...
                <p class="text-sm text-gray-400 mt-1">Recharges, réservations, participations et remboursements</p>
            </div>
            ...
            <form method="GET" action="{{ route('joueur.historique') }}" class="flex flex-wrap gap-3 mb-6">
                <select name="type" onchange="this.form.submit()"
                    ...
                    <option value="paiement_reservation" {{ request('type') === 'paiement_reservation' ? 'selected' : '' }}>Réservations</option>
                    <option value="paiement_participation" {{ request('type') === 'paiement_participation' ? 'selected' : '' }}>Participations</option>
                    <option value="remboursement" {{ request('type') === 'remboursement' ? 'selected' : '' }}>Remboursements</option>
                </select>
            </form>
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-5 py-4">
                                        @php
                                            $typeConfig = [
                                                ...
                                                'paiement_participation' => ['label' => 'Participation', 'class' => 'bg-violet-50 text-violet-600'],
                                    ...
                                    <td class="px-5 py-4 text-gray-600">
                                        @if($tx->type === 'paiement_reservation' && $tx->transactionnable)
                                            ...
                                            <span class="block font-medium">{{ $res->terrain->nom_terrain ?? 'Terrain' }}</span>
                                            <span class="text-xs text-gray-400">
                                                {{ \Carbon\Carbon::parse($res->date_debut)->format('d/m H:i') }}
                                                → {{ \Carbon\Carbon::parse($res->date_fin)->format('H:i') }}
                                            </span>
                                        @elseif($tx->type === 'paiement_participation' && $tx->transactionnable)
                                            @php $part = $tx->transactionnable; @endphp
                                            <span class="block font-medium">
                                                {{ $part->annonce->reservation->terrain->nom_terrain ?? 'Match' }}
                                            </span>
                                            <span class="text-xs text-gray-400">
                                                {{ \Carbon\Carbon::parse($part->annonce->reservation->date_debut)->format('d/m H:i') }}
                                            </span>
                                        @elseif($tx->type === 'remboursement' && $tx->transactionnable)
                                            @php $part = $tx->transactionnable; @endphp
                                            <span class="block font-medium">
                                                Retrait — {{ $part->annonce->reservation->terrain->nom_terrain ?? 'Match' }}
                                            </span>
                                            <span class="text-xs text-gray-400">
                                                {{ \Carbon\Carbon::parse($part->annonce->reservation->date_debut)->format('d/m H:i') }}
                                            </span>
                                    ...
                                    <td class="px-5 py-4 text-right">
                                        @if($tx->points != 0)
                                            <span class="{{ $tx->points > 0 ? 'text-emerald-500' : 'text-red-400' }} font-medium">
